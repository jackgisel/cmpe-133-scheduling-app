...
for rhs in links_major:
    ...
    for rhs in links:
        ...
        #print(data)
        ''' old structure, might be better to use the next structure.
        school = db.collection('SJSU').document('Majors')
        school.collection(Department).document(Course_Name + " - "+ Section).set(data)
        '''
        dept = db.collection('SJSU').document(Department)
        dep.collection(Course_Name).document(Course_Name + " - "+ Section).set(data)
